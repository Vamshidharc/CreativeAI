...
export async function addCreditsForPlan(userId: string, plan: PlanType) {
  try {
    const credits = CREDITS_PER_PLAN[plan as keyof typeof CREDITS_PER_PLAN];
...
export async function createSubscriptionRecord(
  ...
  try {
    return await withRetry(() =>
      prismaClient.$transaction(async (prisma: { subscription: { create: (arg0: { data: { userId: string; plan: PlanType; paymentId: string; orderId: string; }; }) => any; }; }) => {
